	...
	: Super(ObjectInitializer)
{
	...
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		if (bIsGlobalAsyncScanEnvironment)
		{
			...
			AssetRegistry.OnFilesLoaded().AddUObject(this, &UMyObject::OnAssetRegistryFilesLoaded);
		}
	}
#endif
}
...
class UMyObject* UMyObject::CreateLibrary(UClass* InBaseClass, bool bInHasBlueprintClasses, bool bInUseWeak)
{
	UMyObject* NewLibrary = NewObject<UMyObject>();
...
void UMyObject::UseWeakReferences(bool bSetUseWeak)
{
...
bool UMyObject::AddObject(UObject* NewObject)
{
	if (NewObject == NULL)
	{
		return false;
...
bool UMyObject::RemoveObject(UObject* ObjectToRemove)
{
	if (!bUseWeakReferences)
	{
...
int32 UMyObject::LoadAssetsFromPaths(const TArray<FString>& Paths)
{
	int32 Count = 0;
...
int32 UMyObject::LoadBlueprintsFromPaths(const TArray<FString>& Paths)
{
	int32 Count = 0;
...
int32 UMyObject::LoadAssetDataFromPaths(const TArray<FString>& Paths, bool bForceSynchronousScan)
{
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
	IAssetRegistry& AssetRegistry = AssetRegistryModule.Get();
...
int32 UMyObject::LoadBlueprintAssetDataFromPaths(const TArray<FString>& Paths, bool bForceSynchronousScan)
{
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
	IAssetRegistry& AssetRegistry = AssetRegistryModule.Get();
...
int32 UMyObject::LoadAssetsFromAssetData()
{
	int32 Count = 0;
...
void UMyObject::GetAssetDataList(TArray<FAssetData>& OutAssetData)
{
	OutAssetData = AssetDataList;
}

#if WITH_EDITOR
void UMyObject::OnAssetRegistryFilesLoaded()
{
